...
if [[ ! -d "$outdir_tama/${cell_type}" ]]; then
    ...
    printf "%s\t%s\t%s\t%s\n" ""$stringtie_dir"/"$stringtie_base"_stringtie.bed12" "capped"   "1,1,1" "stringtie" >  file_list_${cell_type_small}.txt
    printf "%s\t%s\t%s\t%s\n" ""$mando_dir"/"$mando_base"_mando.bed12" "capped"  "1,1,1" "mandalorion" >> file_list_${cell_type_small}.txt
    printf "%s\t%s\t%s\t%s\n" ""$isoquant_dir"/"$isoquant_base"_iso.bed12" "capped"   "1,1,1" "isoquant" >> file_list_${cell_type_small}.txt
