...
if [[ ! -d "$outdir_tama/${cell_type}" ]]; then
    ...
    # gene id, trans id for stringtie: otherwise TAMA script cannot work with it
    # and number exons
    ...
    isoquant_tama_gtf="${isoquant_dir}"/${isoquant_base}_for_tama.gtf
    ...
    stringtie_tama_gtf="${stringtie_dir}"/${stringtie_base}_for_tama.gtf
    ...
    "$mando_tama_gtf" \
    "$mando_dir"/"$mando_base"_mando.bed12

    # convert Isoquant assembly to BED12
    python "${tama_tool_path}"/tama_go/format_converter/tama_format_gtf_to_bed12_stringtie.py \
    "$isoquant_tama_gtf" \
    "$isoquant_dir"/"$isoquant_base"_iso.bed12


    # convert Stringtie3 assembly to BED12
    python "${tama_tool_path}"/tama_go/format_converter/tama_format_gtf_to_bed12_stringtie.py \
    "$stringtie_tama_gtf" \
    "$stringtie_dir"/"$stringtie_base"_stringtie.bed12

    # create TXT file of Input GTFs for TAMA merge
    printf "%s\t%s\t%s\t%s\n" ""$stringtie_dir"/"$stringtie_base"_stringtie.bed12" "no_cap"   "1,1,1" "stringtie" >  file_list_${cell_type_small}.txt
    printf "%s\t%s\t%s\t%s\n" ""$mando_dir"/"$mando_base"_mando.bed12" "no_cap"  "1,1,1" "mandalorion" >> file_list_${cell_type_small}.txt
    printf "%s\t%s\t%s\t%s\n" ""$isoquant_dir"/"$isoquant_base"_iso.bed12" "no_cap"   "1,1,1" "isoquant" >> file_list_${cell_type_small}.txt
    
    
    # merge the three assemblies
    # -m 2: NAGNAG sites are exactly 3nt
...
if [[ ! -e ""$outdir_tama"/${cell_type}/${cell_type}_merged_tama_gene_id_04_09_26.gtf" ]]; then
    conda activate pacbio
