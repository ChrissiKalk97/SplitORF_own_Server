...
bam_dir_huvec="/projects/splitorfs/work/PacBio/merged_bam_files/genome_alignment/HUVEC/pbmm2_align"
bam_dir_cm="/projects/splitorfs/work/PacBio/merged_bam_files/genome_alignment/CM/pbmm2_align"
...
outdir_cm_fastp="/projects/splitorfs/work/short_RNA_seq_analysis/short_RNA_April_2025/CM_fastp"
...
if [ ! -e "$bam_dir_cm/merged/cm_merged.bam" ]; then
...
if [ ! -e "$bam_dir_huvec/merged/huvec_merged.bam" ]; then
...
#################################################################################
if [ ! -e "$out_path/HUVEC/HUVEC_strigntie3_assembly.gtf" ]; then
...
if [ ! -e "$out_path/CM/CM_strigntie3_assembly.gtf" ]; then
    /home/ckalk/tools/stringtie-3.0.1.Linux_x86_64/stringtie\
...
if [ ! -d "${stringtie3_dir_raw}"/kallisto/HUVEC_quant ]; then
    mkdir "${stringtie3_dir_raw}"/kallisto/HUVEC_quant

    bash ${script_dir}/kallisto/kallisto_index.sh \
    $out_path/HUVEC/HUVEC_strigntie3_assembly_filtered.gtf \
    ${genome_fasta} \
    "${stringtie3_dir_raw}"/kallisto/HUVEC_strigntie3_assembly_transcriptome.fa \
    "${stringtie3_dir_raw}"/kallisto/index/HUVEC

    bash ${script_dir}/kallisto/kallisto_quantification.sh \
    "${stringtie3_dir_raw}"/kallisto/index/HUVEC.idx \
    ${outdir_fastp} \
    "${stringtie3_dir_raw}"/kallisto/HUVEC_quant
fi


if [ ! -d "${stringtie3_dir_raw}"/kallisto/CM_quant ]; then
    mkdir "${stringtie3_dir_raw}"/kallisto/CM_quant

    bash ${script_dir}/kallisto/kallisto_index.sh \
    $out_path/CM/CM_strigntie3_assembly_filtered.gtf \
    ${genome_fasta} \
    "${stringtie3_dir_raw}"/kallisto/CM_strigntie3_assembly_transcriptome.fa \
    "${stringtie3_dir_raw}"/kallisto/index/CM

    bash ${script_dir}/kallisto/kallisto_quantification.sh \
    "${stringtie3_dir_raw}"/kallisto/index/CM.idx \
    ${outdir_cm_fastp} \
    "${stringtie3_dir_raw}"/kallisto/CM_quant
fi
...
if [ ! -d "${sqanti_dir}"/SQANTI3_QC/CM ]; then
    ...
    ${script_dir}/sqanti3/CM_short_reads.txt \
    "${stringtie3_dir_raw}"/kallisto/CM_quant
fi
