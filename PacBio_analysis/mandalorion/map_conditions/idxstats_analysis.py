...
def main(idx_path):
    ...
    for idx in idx_files:
        ...
        idx_df = idx_df.rename(
            columns={0: 'isoform', 1: 'length', 2: f'{sample}', 3: 'unmapped'})
        idx_dict = dict(zip(idx_df['isoform'], idx_df[f'{sample}']))
        idx_summary_df[f'{sample}'] = idx_summary_df['isoform'].map(
            idx_dict)

    sample = sample.split('_')[0]
    idx_summary_df.to_csv(os.path.join(idx_path, f'{sample}_idx_summary.csv'))
