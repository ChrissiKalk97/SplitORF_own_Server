...
# for my parameters
#if [ ! -e "$out_path/"${cell_type}"/minimap2_align/genome_index/genome_index.mmi" ]; then
    ...
    # minimap2 -d "${out_path}"/"${cell_type}"/minimap2_align/genome_index/genome_index.mmi "${genome_fasta_file}"
# fi
...
for fastq in "${isoseq_reads_dir}"/"${cell_type}"*fastq; do
    ...
    if [ ! -e "${out_path}"/"${cell_type}"/minimap2_align/"${sample}"_minimap2_aligned_genome.bam ]; then
