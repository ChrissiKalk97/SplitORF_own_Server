...
class Region:
    def __init__(self,
                 ...
                 g_start: Optional[int] = None,
                 g_stop: Optional[int] = None,
        ...
        self.t_stop = t_stop
        self.length = self.t_stop - self.t_start

        if self.length < 0:
            raise ValueError("Region Length cannot be negative.")
...
class SplitOrf(Region):
    def __init__(self,
                 ...
                 g_stop: Optional[int] = None,
                 strand: Optional[str] = None,
                 unique_region: Optional[Region] = None,
                 Orf_position: Optional[str] = None
                 ):
        # Call parent class constructor
        super().__init__(OrfID, tID, t_start, t_stop, gID, chromosome,
                         g_start, g_stop, strand
                         )

        # Add new attributes for the subclass
        self.unique_region = unique_region
        self.Orf_position = Orf_position


################################################################################
##
# Load SO pipeline results
##
################################################################################
predicted_SO_ORFs, SO_transcripts = load_SO_results(SO_results)
predicted_SO_ORFs = predicted_SO_ORFs[[
    'OrfTransID', 'OrfIDs', 'OrfStarts', 'OrfPos']]
...
all_predicted_SO_ORFs = predicted_SO_ORFs.explode(
    ['OrfID', 'OrfStart', 'OrfPos'], ignore_index=True)
...
# Create SplitORFs from rows of dataframes


def create_features(row):
    SO_instance = SplitOrf(OrfID=row['OrfID'],
                           tID=row['OrfTransID'],
                           t_start=int(row['OrfStart']),
                           t_stop=int(row['OrfPos'].split('-')[1]))
    return SO_instance


splitorfs_list = []
for _, row in all_predicted_SO_ORFs.iterrows():
    splitorfs_list.append(create_features(row))


################################################################################
##
# Load validated SplitOrfs
##
################################################################################
...
all_predicted_SO_ORFs['ValidationCount'] = all_predicted_SO_ORFs.iloc[:, range(
    6, 24)].sum(axis=1, numeric_only=True)
...
ax.pie([nr_validated_SO, nr_SO_not_validated, nr_SO_no_UR],
       ...
       autopct=lambda p: '{:.0f}'.format(p * total_nr_SO / 100), colors=['#CC79A7', '#FFC500', '#75C1C5'])
