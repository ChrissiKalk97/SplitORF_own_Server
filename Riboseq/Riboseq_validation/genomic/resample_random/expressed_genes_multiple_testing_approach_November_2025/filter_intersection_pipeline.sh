...
while getopts 'b:c:e:g:hi:o:s:t:u:' option; do
  case "$option" in
    b)
        bam="$OPTARG"
        ;;
...
if [ ! -e  "$output_star"/NMD_genome/${sample_name}/${sample_name}_NMD_intersect_counts_sorted.bed ]; then
    ...
    # -b $bam > "$output_star"/NMD_genome/${sample_name}/3_primes_genomic_merged_numbered_${sample_name}_coverage.tsv

    # # set to 1 million to not filter
