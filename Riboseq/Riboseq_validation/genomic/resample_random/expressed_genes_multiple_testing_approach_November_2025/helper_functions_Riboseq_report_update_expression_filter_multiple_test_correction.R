...
calculate_background_rank_counts <- function(unique_region_type = `?`(character), path) {
    ...
    for (sample in unique(samples)){
        ...
        mean_bas_dev_z_scores <- mean_absolute_deviation(random_ranked_df)
        filter_mask <- mean_bas_dev_z_scores < 3
        # print(paste0("length filter mask", as.character(length(filter_mask))))
        # print(paste0("head filter maks", head(filter_mask)))
        # print(paste0("keep filter maks", as.character(sum(filter_mask))))
        random_ranked_df <- random_ranked_df[filter_mask,]
...
calculate_fdr_urs <- function(intersect_frames_list, random_ranked_dfs){
    ...
    for (i in seq_along(intersect_frames_list$frames_processed)){
      ...
      ur_rr_concat_df <- ur_rr_concat_df %>%
        arrange(desc(relative_count))
      ur_rr_concat_df <- ur_rr_concat_df %>% mutate(cumsum_ur = cumsum(region_type == "UR"),
                                                    cumsum_rr = cumsum(region_type == "RR")
      )
      ur_rr_concat_df$q_value <- ur_rr_concat_df$cumsum_rr/ur_rr_concat_df$cumsum_ur
      # this is correct: cumulative minimum takes the minimum up to this point
      # we need to start from the back, so we need to rev(q_value)
      # and then reverse all of the cummin again, otherwise the first and last position are swapped
      ur_rr_concat_df <- ur_rr_concat_df %>% mutate(cummin_q_value = rev(cummin(rev(q_value))),
                                                    significant = ifelse(cummin_q_value < 0.05, ifelse(num_reads > 2, 1, 0), 0))
      ...
      genomic_frame <- merge(genomic_frame, ur_df[, c("name", "cummin_q_value", "significant")],
                         by = "name", 
                         all.x = TRUE) 
...
get_top_5_unique_regions <- function(dataframes_list_genomic) {
  ...
  for (unique_region_frame in dataframes_list_genomic) {
    ...
    if (dim(unique_region_frame)[1] > 0) {
      ...
      unique_region_frame <- unique_region_frame[, c("ID", "len", "chr_unique", "start", "stop", "num_reads", "relative_count", "cummin_q_value")]
...
Split_ORFs_validation <- function(dataframes, unique_names_per_sample, path) {
    ...
    for (unique_region_frame in dataframes) {
        unique_region_frame <- unique_region_frame[, c("name", "distinct_ribo_count", "len", "chr_unique", 
                                                       ...
                                                       "cummin_q_value", "relative_count", "significant")]
