...
def get_random_bed_ids(lengthdistribution, UTR_bed_file, outname):
    ...
    random_regions_lengths = [(interval.name, interval.length, interval.chrom, interval.start, interval.end, interval.strand) for interval in three_prime_UTRs]
    randomlist=[]
    # random_choices = []
    with open(outname, 'w') as out:
        for i in range(len(lengthdistribution)):
            ...
            # radnom_choices.append(random_seq)
            ...
            if required_length + 18 < UTR_length and random_seq[5] == '1':
                start = random.randint(int(random_seq[3]) + 18, UTR_length - required_length + int(random_seq[3]))
            elif  required_length + 18 < UTR_length and random_seq[5] == '-1':
                start = random.randint(int(random_seq[3]),  UTR_length - required_length + int(random_seq[3]) - 18)
            ...
            random_regions_lengths = [(name, length, chrom, start, stop, strand) for name, length, chrom, start, stop, strand in random_regions_lengths if name != random_key]
