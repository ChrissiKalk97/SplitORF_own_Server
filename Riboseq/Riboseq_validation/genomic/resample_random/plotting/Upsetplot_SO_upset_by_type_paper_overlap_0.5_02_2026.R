...
cell_type_to_sample_dict <- list(
    ...
    "glioblastoma U251" = c("SRR10533453", 
                            ...
                            "SRR10533459", 
                            "SRR105334560"),
     "glioblastoma U343" = c("SRR105334561",
                            "SRR105334562", 
                            "SRR105334563", 
                            "SRR105334564", 
                            "SRR105334565", 
...
upset_list_gen_regions_per_cell_type <- function(cell_type_to_sample_dict, validated_so_df){
    ...
    for (cell_type in names(cell_type_to_sample_dict)) {
        ...
        keep_cols <- c(samples_of_cell_type, c("OrfID"))
        print(keep_cols)
        df_sample <- validated_so_df[,keep_cols]

        if (is.data.frame(df_sample) && nrow(df_sample) > 0){
            print(rowSums(df_sample == "True", na.rm = TRUE))
            df_sample_validated <- df_sample[rowSums(df_sample == "True", na.rm = TRUE) > 0, "OrfID"]
            upsetlist_genomic[[cell_type]] <- unique(df_sample_validated$OrfID) 
        }
...
get_upset_list_from_csv <- function(path, coarse) {
    file_path <-  file.path(path, "validated_so_df.csv")
