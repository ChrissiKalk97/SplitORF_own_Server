...
background <- calculate_background_threshold(unique_region_type, path)
# print(background)
background <- print_thresholds(unique_region_type, background, path)
...
```{r, message = FALSE}
counts_and_names_list <- count_unique_regions_above_threhsold(list_dataframes, threshold)
...
upset(fromList(upsetlist), 
      order.by = "freq",
      nsets = nr_sets,
      point.size = 1.5,   # grows with number of sets
      line.size  = 1.2,   # grows with number of sets
      set_size.show = TRUE,
...
upset(fromList(upsetlist_genomic), order.by = "freq", nsets = length(names(list_dataframes)), point.size = 2.25, line.size = 1.5, set_size.show = TRUE,
...
upsetlist <- counts_and_names_list$unique_names
upsetlist <- lapply(upsetlist, function(x) x)
upset(fromList(upsetlist), order.by = "freq", nsets = length(names(list_dataframes)), point.size = 2.25, line.size = 1.5, set_size.show = TRUE,
      set_size.scale_max = max(nr_regions) + 100, 
...
```{r, echo=FALSE, message=FALSE}
svg(file.path(path, paste('upset', unique_region_type, 'UR.png', sep = '_')))
names(counts_and_names_list$unique_names_genomic) <- names(list_dataframes)
upsetlist_genomic <- counts_and_names_list$unique_names_genomic
upsetlist_genomic <- lapply(upsetlist_genomic, function(x) x)
upset(fromList(upsetlist_genomic), order.by = "freq", nsets = length(names(list_dataframes)), point.size = 2.25, line.size = 1.5, set_size.show = TRUE,
...
```{r, message = FALSE}
counts_and_names_list$frames_processed <- get_print_ORF_ranks(SO_pipe_path, counts_and_names_list$frames_processed)
names(counts_and_names_list$frames_processed) <- names(list_dataframes)
names(counts_and_names_list$genomic_frames) <- names(list_dataframes)
write_csv(counts_and_names_list$genomic_frames, upsetlist_genomic, counts_and_names_list$frames_processed, upsetlist, path)
```


Print the unique regions where at least 2 unique regions on the same transcript were found.
```{r}
Split_ORFs_validation(counts_and_names_list$frames_processed, upsetlist, path)
```
