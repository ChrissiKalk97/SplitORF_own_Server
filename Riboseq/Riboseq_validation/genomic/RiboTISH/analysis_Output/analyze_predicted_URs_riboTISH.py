...
for pred_ORFs_file in glob.glob("*_framebest_all.txt"):
    ...
    print(SO_df.sort_values('RiboPvalue')[['Gid', 'Tid', 'Symbol', 'GenomePos', 'TisType', 'RiboPvalue', 'InFrameCount']])
    SO_df = SO_df.groupby('GenomePos').agg('first')
    SO_df = SO_df.reset_index()
    SO_df.sort_values('RiboPvalue')[['Gid', 'Tid', 'Symbol', 'GenomePos', 'TisType', 'RiboPvalue', 'InFrameCount']].to_csv(f'{sample_name}_UR_translation_RiboTISH.csv', index=False)