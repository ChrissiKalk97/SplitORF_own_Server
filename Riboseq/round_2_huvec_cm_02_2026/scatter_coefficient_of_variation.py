...
def main(path_to_cv_csvs_1, path_to_cv_csvs_2,
         out_path, condition, color='blue'):
    def plot_scatter(cv_df_concat, out_path, condition, color):
        ...
        ax.set_title(
            f'coefficient of variation in transcripts in {condition}: round 2 vs round1')

        plt.xlim(0, 10)
        plt.ylim(0, 10)

        plt.savefig(os.path.join(out_path, f'{condition}_cv_scatter.svg'),
                    format='svg', dpi=300, bbox_inches='tight')
        plt.close()

    for csv in os.listdir(path_to_cv_csvs_1):
        if csv.endswith('.csv') and condition in csv.upper():
    ...
    print(f'Nr of transcripts with CPM > 5 in both datasets {condition}:', len(
        tids_keep))
    ...
    plot_scatter(cv_df_concat, out_path, condition, color)
