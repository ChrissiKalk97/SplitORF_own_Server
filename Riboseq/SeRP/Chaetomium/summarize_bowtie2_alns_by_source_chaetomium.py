...
def plot_summary(summary_mapping_stats_df, outdir, raw=False, figsize=(12, 6)):
    ...
    summary_mapping_stats_df_t = summary_mapping_stats_df_t.rename(columns={'Chaetomium_thermophilum_protein_coding': 'protein_coding',
                                                                            'Chaetomium_thermophilum_tRNA': 'tRNA',
                                                                            'Chaetomium_thermophilum_rRNA': 'rRNA'})
    ...
    plt.bar(sample_names, rRNA_df,
            color='#40A3CD', label='rRNA')
    plt.bar(x, tRNA_df, bottom=rRNA_df,
            color='#019c91', label='tRNA')
    plt.bar(x, mRNA_df, bottom=tRNA_df +
            rRNA_df, color='#c73832', label='mRNA')

    # add percentage labels
    add_labels(x, mRNA_df,  tRNA_df + rRNA_df)
    add_labels(x, tRNA_df, rRNA_df)
    add_labels(x, rRNA_df, None)

    if raw:
        unmapped_filtered_df = summary_mapping_stats_df_t["filtered_unmapped"]
        plt.bar(x, unmapped_filtered_df, bottom=tRNA_df + rRNA_df + mRNA_df,
                color='#FDDA0D', label='filtered_unmapped')
        add_labels(x, unmapped_filtered_df,
                   tRNA_df + rRNA_df + mRNA_df)
