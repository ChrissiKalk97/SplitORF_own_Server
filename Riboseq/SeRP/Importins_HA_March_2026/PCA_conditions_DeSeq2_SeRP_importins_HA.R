...
png(
    file.path(
        in_dir,
        "PCA_transcriptome_counts_greater_10_rlog_mRNA_batch.png"
    ),
    width = 800,
    height = 800
)
print(plotPCA(rid_mRNA, intgroup = c("batch")))
dev.off()
...
write.csv(counts(dds_mRNA),
file=file.path(in_dir, "DEGs", "all_counts_unfiltered.csv"),
...
perform_diff_analysis <- function(dds_mRNA, treatment, control, outname, in_dir, lfc = 0.5)
 {
    ...
    write.csv(res_treat,
    file=file.path(in_dir, "DEGs", paste(str_split(outname, "\\.")[[1]][1], "all_results.csv", sep = "_")),
    row.names=TRUE, quote=FALSE)
