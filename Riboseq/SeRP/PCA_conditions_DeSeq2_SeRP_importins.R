...
################################################################################
idx_files = c()
if(!(identical(list.files(in_dir, 
...
merged_counts_matrix_mRNA <- merged_counts_matrix_mRNA[rowSums(merged_counts_matrix_mRNA) > 10,]
...
perform_diff_analysis <- function(dds_mRNA, treatment, control, outname, in_dir){
    ...
    # write output
    writeLines(rownames(res_treat_pval[res_treat_pval$padj < 0.05, ]),
    file.path(in_dir, outname))
}
