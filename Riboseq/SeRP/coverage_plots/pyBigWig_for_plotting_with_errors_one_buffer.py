...
def main(path_to_bw_files, mane_transcripts_cds_bed, transcripts_to_plot_txt,
    ...
    def plot_enrichment(transcript_df, out_path, transcript, importin, background, color, window_size=21):
        ...
        plt.tight_layout()
        if puro == 'Puro':
            plt.savefig(os.path.join(out_path, f'{transcript}_enrichment_{importin}_over_{background}_{puro}_CDS_only_from_whole_trans_ws{window_size}.svg'),
                        format='svg', dpi=300, bbox_inches='tight')
        else:
            plt.savefig(os.path.join(out_path, f'{transcript}_enrichment_{importin}_over_{background}_CDS_only_from_whole_trans_ws{window_size}.svg'),
                        format='svg', dpi=300, bbox_inches='tight')
        plt.close()

    def calculate_and_plot_enrichment_df(MANE_CDS_coords, bws_to_average, out_path, importin, background, color, window_size=21):
    ...
    calculate_and_plot_enrichment_df(
        MANE_CDS_coords, bws_to_average, out_path, importin, background, color, window_size)
