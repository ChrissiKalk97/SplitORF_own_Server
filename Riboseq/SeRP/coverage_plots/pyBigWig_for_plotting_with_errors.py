...
# path_to_bw_files = '/projects/serp/work/Output/April_2025/importins/transcriptome_mapping/filtered/q10/enrichment_plots_CDS'
...
# out_path = '/projects/serp/work/Output/April_2025/importins/transcriptome_mapping/filtered/q10/enrichment_plots_CDS/impA_B_0_5_M_and_input_DEG_plots'


def main(path_to_bw_files, MANE_transcripts_CDS_bed, transcripts_to_plot_txt,
         out_path, importin='A2', background='Input', Puro=False):
    ...
    def plot_enrichment(transcript_df, out_path, transcript, importin, background):
        plt.figure(figsize=(10, 5))

        # Plot the average line
        plt.plot(transcript_df['CDS_position'],
                 transcript_df['average_ratio'], label='Average Ratio', color='blue')
        ...
        plt.xlabel('CDS Position (bp)')
        plt.ylabel('Ratio A2/Input')
        plt.title(f'SeRP enrichment signal across {transcript} CDS')
        plt.legend()
        plt.tight_layout()

        plt.savefig(os.path.join(out_path, f'{transcript}_enrichment_{importin}_over_{background}_CDS_only.svg'),
                    format='svg', dpi=300, bbox_inches='tight')
        plt.close()

    def calculate_and_plot_enrichment_df(MANE_CDS_coords, bws_to_average, out_path, importin, background):
        for transcript in MANE_CDS_coords.index:
            ...
            for bw_object in bws_to_average:
                ...
                bw_values = [bw_values_binned[i//3]
                             for i in range(len(bw_values))]

                bw_values_smoothed = [
                    sum(bw_values[i-9:i+12])/21 for i in range(9, len(bw_values)-9, 3)]

                bw_values_binned_smoothed = [bw_values_smoothed[i//3]
                                             for i in range(len(bw_values)-18)]

                observed_values.append(bw_values_binned_smoothed)
            ...
            transcript_df = pd.DataFrame({'CDS_position': range(start, stop),
            ...
            plot_enrichment(transcript_df, out_path,
                            transcript, importin, background)
    ...
    calculate_and_plot_enrichment_df(
        MANE_CDS_coords, bws_to_average, out_path, importin, background)
