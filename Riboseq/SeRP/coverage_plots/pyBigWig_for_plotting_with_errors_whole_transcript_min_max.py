...
def main(path_to_bw_files, mane_transcripts_cds_bed, transcripts_to_plot_txt,
    ...
    def plot_enrichment(transcript_df, out_path, transcript, importin, background, color):
        ...
        plt.plot(transcript_df['CDS_position'],
                 transcript_df['average_ratio'], label='Average Enrichment', color=color)
        ...
        plt.tight_layout()
        if puro == 'Puro':
            plt.savefig(os.path.join(out_path, f'{transcript}_enrichment_{importin}_over_{background}_{puro}_CDS_only_from_whole_trans_min_max.svg'),
                        format='svg', dpi=300, bbox_inches='tight')
        else:
            plt.savefig(os.path.join(out_path, f'{transcript}_enrichment_{importin}_over_{background}_CDS_only_from_whole_trans_min_max.svg'),
                        format='svg', dpi=300, bbox_inches='tight')
        plt.close()
